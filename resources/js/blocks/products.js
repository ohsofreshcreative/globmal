document.querySelectorAll('section.b-products').forEach(function (root) {
  const nav = root.querySelector('.js-products-nav');
  if (!nav) return;

  const products = Array.from(nav.querySelectorAll('.tab_btn'));
  if (!products.length) return;

  let active = products.findIndex(t => t.classList.contains('active'));
  if (active < 0) active = 0;
  products.forEach((btn, i) => btn.classList.toggle('active', i === active));
