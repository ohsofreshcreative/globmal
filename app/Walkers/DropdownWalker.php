...
class DropdownWalker extends Walker_Nav_Menu
{
    ...
    {
        $classes = 'absolute z-10 mt-2 w-max origin-top-right bg-transparent border-third rounded-[40px] focus:outline-none';
    ...
    public function start_el(&$output, $item, $depth = 0, $args = null, $id = 0)
    {
        ...
        if ($depth === 0 && $has_children) {
            ...
            $output .= '<a href="' . esc_attr($item->url) . '" class="inline-flex items-center gap-x-1 text-sm text-white font-medium hover:text-indigo-600">';
        ...
        // Case 2: Pozostałe elementy (zwykłe linki na górze lub linki wewnątrz dropdownu)
            ...
            $link_classes = '';
            if ($depth > 0) {
                $link_classes = 'block text-white rounded-xl px-6 py-4 text-sm text-gray-700 hover:bg-primary';
            } else {
